...
void keyPressed() {
  ...
  case 'g'://show generations
    ...
    genPlayerTemp = pop.genPlayers.get(upToGen).cloneForReplay();
    break;
  case 'n'://if we show nothing it works faster
...
//updates obstacles every frame
void updateObstacles() {
  obstacleTimer ++;
  speed += 0.002;
  if (obstacleTimer > minimumTimeBetweenObstacles + randomAddition) { //adds obstacles from time to time
    addObstacle();
  }
  groundCounter ++;
...
//moves obstacles at the speed the game is running 
void moveObstacles() {
  println(speed);
...
void addObstacle() {
  ...
  int tempInt;
  tempInt = floor(random(3)); // 3 types of birds &  3 types of cacti - generate the type
  if (lifespan > 1000 && random(1) < 0.15) { //adds a bird sometimes (chance can be configured)
    Bird temp = new Bird(tempInt);
    birds.add(temp);
  } else {//add cactus in other cases
