...
class Player {
  ...
  int lifespan = 0;//how long the player lived, used calculate fitness
  ...
  void look() {
    if (!replay) {
      ...
      vision[4] = speed; //speed of the player
      vision[5] = posY;  //height of the player


      if (minIndex == -1) {//no obstacles on the screen
        vision[0] = 0; //distance to next obstacle
        vision[1] = 0; //height of the next obstacle
        vision[2] = 0; //witdth of an obstacle
        vision[3] = 0; //type of bird
        vision[6] = 0; //gap between the 2 next obstacles
      } else {

        vision[0] = 1.0/(min/10.0);
        if (berd) {
          vision[1] = birds.get(minIndex).h; //get height
          vision[2] = birds.get(minIndex).w; //get width
          if (birds.get(minIndex).typeOfBird == 0) {
            vision[3] = 0;
          } else {
            vision[3] = birds.get(minIndex).posY; //get type of bird
          }
        } else {
        ...
        if (minIndex == -1) {//if there is only 1 object on the screen
          vision[6] = 0;
        } else {
          vision[6] = 1/(min - closestDist);
        }
      }
    } else {//used for replays
      float temp = 0;
      float min = 10000;
      int minIndex = -1;
      boolean berd = false; 
  ...
  //gets the output of the NN then converts them to actions
  void think() {
  ...
  //remove randomness from replays
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------------------------
  //if replaying then the dino has local obstacles (the ones we stored when he was running)
